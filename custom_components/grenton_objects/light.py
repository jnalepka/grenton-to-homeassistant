...
class GrentonLight(LightEntity):
    ...
    async def async_turn_on(self, **kwargs):
        try:
            ...
            _LOGGER.debug(f"DEBUG light.py TURN ON REQUEST - {self._name}, HA status: {self._state}")
    ...
    async def async_turn_off(self, **kwargs):
        try:
            ...
            _LOGGER.debug(f"DEBUG light.py TURN OFF REQUEST - {self._name}, HA status: {self._state}")
    ...
    async def async_update(self):
        try:
            ...
            _LOGGER.debug(f"DEBUG light.py START UPDATE REQUEST - {self._name}, HA status: {self._state}")
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self._api_endpoint}", json=command) as response:
                    ...
                    (f"DEBUG light.py GRENTON STATE UPDATE - {self._name}, Grenton status: {data.get("status")}")
